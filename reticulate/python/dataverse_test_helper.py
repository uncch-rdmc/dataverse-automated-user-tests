...
def set_dataverse_metadata(add_string=''):
...
def test_dataverse_metadata(add_string=''):
  ...
  expect_identical(self.sesh.find_element('xpath','//*[@id="breadcrumbLnk0"]').text, dv_props['host_dataverse'])
  expect_identical(self.sesh.find_element('xpath','//*[@id="dataverseHeader"]/div/div/a/h1').text, add_string+dv_props['name'])
  expect_identical(self.sesh.find_element('xpath','//*[@id="dataverseHeader"]/div/div/span[1]').text, '('+add_string+dv_props['affiliation']+')')
  expect_identical(sesh.current_url), paste(dv_server_url,'/dataverse/', add_string, dv_props['identifier'], '/')
  expect_identical(self.sesh.find_element('xpath','//*[@id="dataverseDesc"]').text, add_string+dv_props['description'])
  ...
  expect_identical(self.sesh.find_element('xpath','//*[@id="dataverseForm:dataverseCategory"]/option[@selected]').text, dv_props['category'])
  expect_identical(self.sesh.find_element('xpath','//*[@id="dataverseForm:j_idt271:0:contactEmail"]').get_attribute('value'), add_string+dv_props['email'])
